...
def build_full_explanation_set(report: dict) -> dict:
    if report.get("rules"):
        grouped = {"PASS": [], "FAIL": [], "REVIEW": [], "N/A": []}
        for rule in report["rules"]:
            grouped[rule["status"]].append({
                "rule_id": rule["rule_id"], "rule_violated": rule["check"],
                "evidence": rule.get("detected_text") or "No image evidence available.",
                "suggested_action": rule.get("explanation", "Review this checklist item."),
                "status": rule["status"],
            })
        return {"rule_explanations": grouped, "field_explanations": {}, "readability_explanations": []}
